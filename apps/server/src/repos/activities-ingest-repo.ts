...
const INSERT_ACTIVITY = `
  INSERT INTO activities (
    ...
    token_in2_address, token_in2_symbol, token_in2_decimals,
    token_out2_address, token_out2_symbol, token_out2_decimals,
    amount_in2_raw, amount_out2_raw, executed_in2_raw, executed_out2_raw,
    usd_in_est, usd_out_est, usd_fee_est,
    usd_network_gas_est, usd_venue_fee_est, usd_vex_fee_est, usd_destination_prepay_est,
    usd_source,
...
const PROMOTE_PENDING_ACTIVITY = `
  UPDATE activities SET
    ...
    token_in2_address = COALESCE($7, token_in2_address),
    token_in2_symbol = COALESCE($8, token_in2_symbol),
    token_in2_decimals = COALESCE($9::smallint, token_in2_decimals),
    token_out2_address = COALESCE($10, token_out2_address),
    token_out2_symbol = COALESCE($11, token_out2_symbol),
    token_out2_decimals = COALESCE($12::smallint, token_out2_decimals),
    executed_in2_raw = COALESCE($13, executed_in2_raw),
    executed_out2_raw = COALESCE($14, executed_out2_raw),
    usd_in_est = COALESCE($15::numeric, usd_in_est),
    usd_out_est = COALESCE($16::numeric, usd_out_est),
    usd_fee_est = COALESCE($17::numeric, usd_fee_est),
    usd_network_gas_est = COALESCE($18::numeric, usd_network_gas_est),
    usd_venue_fee_est = COALESCE($19::numeric, usd_venue_fee_est),
    usd_vex_fee_est = COALESCE($20::numeric, usd_vex_fee_est),
    usd_destination_prepay_est = COALESCE($21::numeric, usd_destination_prepay_est),
    usd_source = COALESCE($22, usd_source),
    failure_code = COALESCE($23, failure_code),
    client_confirmed_at = COALESCE($24::timestamptz, client_confirmed_at),
    client_observed_at = COALESCE($25::timestamptz, client_observed_at),
...
async function insertActivity(
  ...
  const inserted = await client.query<VerificationStateRow>(INSERT_ACTIVITY, [
    ...
    event.tokenIn2?.address ?? null,
    event.tokenIn2?.symbol ?? null,
    event.tokenIn2?.decimals ?? null,
    event.tokenOut2?.address ?? null,
    event.tokenOut2?.symbol ?? null,
    event.tokenOut2?.decimals ?? null,
    event.amountIn2Raw,
    event.amountOut2Raw,
    event.executedIn2Raw,
    event.executedOut2Raw,
    event.usdInEst,
    event.usdOutEst,
    event.usdFeeEst,
    event.usdNetworkGasEst,
    event.usdVenueFeeEst,
    event.usdVexFeeEst,
    event.usdDestinationPrepayEst,
    event.usdSource,
    event.txHash,
    event.failureCode,
    event.createdAt,
